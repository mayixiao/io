...
import com.zzxy.common.util.ConvertUtils;

public class MyBeiBao {
    ...
    //capacity需要比较的数
	public void near(List<Map> list, List<Integer> result, int start, BigDecimal capacity, boolean minShort) {

		// 不计算最短路径，那么得到相等之后立即跳出
		if (!minShort) {
			BigDecimal count = count(result);//结果集合
			if (count == capacity) {
				return;
			}
		}
		...
		for (int i = 0; i < list.size(); i++) {
			BigDecimal zjeXm= ConvertUtils.createBigDecimal (list.get(i).get("zj"));
